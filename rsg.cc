...
/**
 * Takes a reference to a map grammar, const reference to a string key for
 * grammar (at first invoke it's always <start>) and string of result
 * random sentence.
 */


static string getRandomSenteces(map<string, Definition>& grammar, const string& key, string result)
{
  Definition definition = grammar[key];
  Production definition_expansion = definition.getRandomProduction();
  vector<string> phrases = definition_expansion.getPhrases();
  Production::const_iterator curr_ph = phrases.begin();
  
  while (curr_ph != phrases.end()) {
    if ((*curr_ph).compare(0, 1, "<") == 0) 
      result = getRandomSenteces(grammar, (*curr_ph), result); 
    else 
      result += (*curr_ph) + " ";
...
int main(int argc, char *argv[])
{
  ...
  cout << "Here three random sentences: " << endl;

  for (int i = 0; i < 3; ++i)
  {
    cout << getRandomSenteces(grammar, "<start>", "") << endl;
  }
